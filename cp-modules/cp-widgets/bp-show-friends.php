...
define ( 'BP_SF_PLUGIN_URL', WP_PLUGIN_URL . '/communitypress/cp-modules/cp-widgets/' );
define ( 'BP_SF_PLUGIN_DIR', WP_PLUGIN_DIR . '/communitypress/cp-modules/cp-widgets/' );
define ( 'BP_SF_VERSION', '1.1.1' );
...
class BP_Show_Friends_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		?>
		<?php if(bp_is_member() || is_user_logged_in()):?>
			<?php
			extract( $args );
			echo $before_widget;
			echo $before_title;
			if(!bp_is_member() || $bp->loggedin_user->id == $bp->displayed_user->id) printf( __( "My Friends - <a href='%s'>All</a>", 'bp-show-friends' ), $user_all_friends_url);
...
function  bp_show_friends_widget_list($limit=15, $nomember=false){
	global $bp;
    ...
    <?php // JLL_MOD - Fixed for privacy settings ?>
<?php if ( bp_privacy_filter("friends") ) : ?>
		<?php echo 'This information is avaliable only for friends'; ?>
<?php else : ?>
	<div id="friends-container" style="padding-top:5px">
	    <?php if ( bp_has_members( 'user_id='.$user_id.'&type=active&per_page='.$limit.'&max='.$limit.'&populate_extras=0' ) ) : ?>
...
function bp_show_friends_get_type_to_display(){
  global $bp;
  if(bp_is_member()) $user_id = $bp->displayed_user->id;
  ...
  if($type=='offline') $args = 'user_id='.$user_id.'&type=active&per_page='.$max.'&max='.$max.'&populate_extras=0';
  else $args = 'user_id='.$user_id.'&type=online&per_page='.$max.'&max='.$max.'&populate_extras=0';
...
function bp_show_friends_custom_js(){
  if ( bp_is_member() || is_user_logged_in()){
    ?>
