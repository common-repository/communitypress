...
function bp_activity_at_message_notification( $content, $poster_user_id, $activity_id ) {
	...
	foreach( (array)$usernames as $username ) {
		...
		if ( 'no' != get_user_meta( $receiver_user_id, 'notification_activity_new_mention', true ) ) {
			...
			// JLL_MOD - fixed content
			$precontent = preg_replace('%<span class="atmentionremoval">@.+?</span>%', '', $content);
			...
			// JLL_MOD - fixed for wall post
			$subject  = '[' . $sitename . '] ' . sprintf( __( '%s posted something on your wall', 'buddypress' ), $poster_name );

$message = sprintf( __(
// JLL_MOD - fixed for wall post
'%s posted something on your wall:
