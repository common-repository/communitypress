...
function bp_core_add_admin_bar_css() {
	...
	if ( ( bp_core_is_multisite() && $current_blog->blog_id != BP_ROOT_BLOG ) || is_admin() ) {
		...
		if ( file_exists( WP_CONTENT_DIR . '/themes/' . $stylesheet . '/_inc/css/adminbar.css' ) )
			...
			wp_enqueue_style( 'bp-admin-bar', apply_filters( 'bp_core_admin_bar_css', WP_PLUGIN_URL . '/communitypress/cp-themes/cp-default/_inc/css/adminbar.css' ) );
...
?>
	...
	<style type="text/css">
	/* // JLL_MOD - fixed .jcrop-holder { float: left;  */
		.jcrop-holder { float: right; margin: 0 20px 20px 0; text-align: left; }
