<?php
// JLL_MOD - fixes throughout entire file
/**
...
class bpModBackend extends bpModeration{
	...
	function  __construct() {
		...
		if( isset($this->options['generate_test_data']) ){
			...
			add_action( 'admin_menu', array( bpModLoader, 'test_data' ), 2 );
		}
	}
	...
	function rightnow_widget_section() {
		...
		if ( $awating = (int)$wpdb->get_var( "$sql WHERE status IN ('new','warned')" ) )
		{
			...
			$text .= sprintf( _n(
				'<a href="%1$s">%2$d sitewide content</a> is awaiting your moderation',
				'<a href="%1$s">%2$d sitewide contents</a> are awaiting your moderation',
		...
		elseif ( $total = (int)$wpdb->get_var( $sql ) )
		{
			...
			$text .= sprintf( _n(
				'there are no sitewide contents awaiting your moderation, <a href="%1$s">%2$d reported content</a> has already been viewed.',
				'there are no sitewide contents awaiting your moderation, <a href="%1$s">%2$d reported contents</a> have already been viewed.',
				$total, 'bp-moderation'), $link, $total
			);
		}
		else
		{
			$text .= __( 'no sitewide content has ever been reported.','bp-moderation' );
		}
...
// JLL_MOD - fixed
		...
		$view = ( !empty($_GET['view']) && in_array( $_GET['view'], array_keys($views) ) ) ? $_GET['view'] :  $defview;
		
		// JLL_MOD - fixed
		$h2 = '<h2><span class="nav-tab nav-tab-title">'.__( 'Moderation', 'bp-moderation' ).'</span>';
		...
		foreach ($messagges_strings as $arg_key => $tr) {
			if( isset($_GET[$arg_key]) ){
				$message = is_numeric($_GET[$arg_key]) ? _n( $tr[0], $tr[1], $_GET[$arg_key], 'bp-moderation') : $tr[0];
	...
	?>
	<form id="bpmod-contents-query" class="bpmod-form-query" action="admin.php" method="get">
		...
		<fieldset>
			...
			<div class="column">
				...
				<dt>
					<input id='filter-item_type' <?php echo isset($_GET['filters']['active_filters']['item_type']) ? $chk : ''
					?> name='filters[active_filters][item_type]' type='checkbox' />
					<label for='filter-item_type' ><?php _e('Content types','bp-moderation') ?></label>
				</dt>
				<dd>
					<ul>
<?php				foreach ($this->content_types as $slug => $ct) : ?>
						<li><input id='type_<?php echo $slug ?>'<?php echo isset($_GET['filters']['item_type'][$slug]) ? $chk : ''
							?> name='filters[item_type][<?php echo $slug ?>]' type='checkbox' />
						<label for='type_<?php echo $slug ?>' ><?php echo $ct->label ?></label></li>
<?php				endforeach ?>
					</ul>
				</dd>
				<dt>
					<input id='filter-status' <?php echo isset($_GET['filters']['active_filters']['status']) ? $chk : ''
					?> name='filters[active_filters][status]' type='checkbox' />
					<label for='filter-status' ><?php _e('Status','bp-moderation') ?></label>
				</dt>
				<dd>
					<ul>
<?php				foreach ($this->content_stati as $slug => $label) : ?>
						<li><input id="<?php echo $slug ?>"<?php echo isset($_GET['filters']['status'][$slug]) ? $chk : ''
							?> name="filters[status][<?php echo $slug ?>]" type="checkbox">
						<label for="<?php echo $slug ?>"><?php echo $label ?></label></li>
<?php				endforeach ?>
					</ul>
				</dd>
				<dt>
					<input id='filter-flags' <?php echo isset($_GET['filters']['active_filters']['flags']) ? $chk : ''
					?> name='filters[active_filters][flags]' type='checkbox' />
					<label for='filter-flags' ><?php _e('Times reported','bp-moderation') ?></label>
				</dt>
				<dd>
					<label for='flags' ><?php
					$input = "<input id='flags' size='4' type='text' name='filters[flags]' value='". (empty($_GET['filters']['flags']) ? '0': $_GET['filters']['flags']) ."' />";
					echo sprintf(__('Reported at least %s times','bp-moderation'),$input); ?></label>
				</dd>
				<dt>
					<input id='filter-item_author' <?php echo isset($_GET['filters']['active_filters']['item_author']) ? $chk : ''
					?> name='filters[active_filters][item_author]' type='checkbox' />
					<label for='filter-item_author' ><?php _e('Content posted by','bp-moderation') ?></label>
				</dt>
				<dd>
					<input id='item_author' class='line' size='40' type='text' name='filters[item_author]' value='<?php
						echo empty($_GET['filters']['item_author']) ? '': $_GET['filters']['item_author'] ?>' />
					<label for='item_author' ><?php _e('User ids (comma separeted)','bp-moderation') ?></label>
				</dd>
				<dt>
					<input id='filter-reporters' <?php echo isset($_GET['filters']['active_filters']['reporters']) ? $chk : ''
					?> name='filters[active_filters][reporters]' type='checkbox' />
					<label for='filter-reporters' ><?php _e('Content reported by','bp-moderation') ?></label>
				</dt>
				<dd>
					<input id='reporters' class='line' size='40' type='text' name='filters[reporters]' value='<?php
						echo empty($_GET['filters']['reporters']) ? '': $_GET['filters']['reporters'] ?>' />
					<label for='reporters' ><?php _e('User ids (comma separeted)','bp-moderation') ?></label>
				</dd>
			</div>
			<div class="column">
...
?>
					...
					$asc = 'DESC' == @$_GET['order'][$i]['dir'] ? 'DESC' : 'ASC' ; ?>
						...
						<select name="order[<?php echo $i ?>][by]">
							<option<?php selected('none',$orby) ?> value="none"><?php _e('none','bp-moderation') ?></option>
							<option<?php selected('flags',$orby) ?> value="flags"><?php _e('times reported','bp-moderation') ?></option>
							<option<?php selected('first_flag',$orby) ?> value="first_flag"><?php _e('first time reported','bp-moderation') ?></option>
							<option<?php selected('last_flag',$orby) ?> value="last_flag"><?php _e('last time reported','bp-moderation') ?></option>
							<option<?php selected('item_id',$orby) ?> value="item_id"><?php _e('item id','bp-moderation') ?></option>
							<option<?php selected('item_id2',$orby) ?> value="item_id2"><?php _e('secondary id','bp-moderation') ?></option>
						</select>
						<select name="order[<?php echo $i ?>][dir]">
							<option<?php selected('ASC',$asc) ?> value="ASC">ASC</option>
							<option<?php selected('DESC',$asc) ?> value="DESC">DESC</option>
						</select>
					</li>
<?php			$i++;
				endwhile;
?>
				...
				<p><label for='limit' ><?php
					...
					echo sprintf(__('Display at most %s contents','bp-moderation'),$input); ?></label></p>
				<input name="submit" type="submit" class="button-primary" value="<?php _e('Query Contents','bp-moderation'); ?>" />
			</div>
		</fieldset>
	</form>
	<div class="clear"></div>
...
?>
	<form id="bpmod-contents-form" class="bpmod-bulk-form" action="admin.php" method="post">
	<div class="tablenav">
		<div class="alignleft actions">
			<select name="bulk-action">
				<option value="-1" selected="selected"><?php _e('Bulk Actions','bp-moderation') ?></option>
				<option value="ignore"><?php _e('Ignore','bp-moderation'); ?></option>
				<option value="mark_moderated"><?php _e('Mark as moderated','bp-moderation'); ?></option>
				<option value="delete"><?php _e('Delete','bp-moderation'); ?></option>
				<option value="mark_spammer"><?php _e('Mark authors as spammers','bp-moderation'); ?></option>
				<option value="unmark_spammer"><?php _e('Mark authors as not spammers','bp-moderation'); ?></option>
				<!--<option value="authors_in_user_view"><?php _e('View authors in users view','bp-moderation'); ?></option>-->
			</select>
			<input type="hidden" name="bpmod-action" value="bulk_contents" />
			<?php wp_nonce_field('bulk_contents'); ?>
			<input type="submit" name="doaction" id="doaction" value="<?php esc_attr_e('Apply','bp-moderation'); ?>" class="button-secondary apply" />
		</div>
	...
	<div class="clear"></div>
	<table id="bpmod-contents-table" class="widefat bpmod-table fixed" cellspacing="0">
	<thead>
		<tr>
			<th class="manage-column column-cb check-column" scope="col"><input type="checkbox"></th>
			<th class="manage-column column-author" scope="col"><?php _e('Author','bp-moderation') ?></th>
			<th class="manage-column column-content" scope="col"><?php _e('Content','bp-moderation') ?></th>
			<th class="manage-column column-flags" scope="col"><?php _e('Flags','bp-moderation') ?></th>
		</tr>
	</thead>
	<tfoot>
		<tr>
...
<?php
	foreach ($results as $content) :
		$ctype = isset($this->content_types[$content->item_type])? $this->content_types[$content->item_type] : false;
...
?>
		<tr class="status-<?php echo $content->status ?>">
			<th class="check-column" scope="row"><input type="checkbox" value="<?php echo $content->content_id ?>" name="bulk_items[]"></th>
...
<?php		if ( $content->item_author && get_userdata($content->item_author) ):
				if (bp_core_is_user_spammer( $content->item_author )): ?>
					<a class="unmark-spammer vim-u" href="<?php echo wp_nonce_url( "admin.php?bpmod-action=mark_unmark_spammer&user_id=$content->item_author&set_spam=0" , 'mark_unmark_spammer' ) ?>" title="<?php _e( 'Mark the author of this content as not spammer', 'bp-moderation' ) ?>"><?php _e( 'Mark as not spammer', 'bp-moderation' ) ?></a>
<?php			else : ?>
					<a class="mark-spammer vim-s" href="<?php echo wp_nonce_url( "admin.php?bpmod-action=mark_unmark_spammer&user_id=$content->item_author&set_spam=1" , 'mark_unmark_spammer' ) ?>" title="<?php _e( 'Mark the author of this content as spammer', 'bp-moderation' ) ?>"><?php _e( 'Mark as spammer', 'bp-moderation' ) ?></a>
<?php			endif;
			elseif ( $content->item_author ): ?>
					<span class="not-a-member" ><?php _e( 'Unregistered', 'bp-moderation' ) ?></span>
<?php		else: ?>
					<span class="not-a-member" ><?php _e( 'Not a member', 'bp-moderation' ) ?></span>
<?php		endif; ?>
				</div>
			</td>
			<td class="column-content">
				<strong><?php _e( 'Status:', 'bp-moderation' ) ?></strong> <?php echo $this->content_stati[$content->status]
				?><strong><?php _e( 'Type:', 'bp-moderation' ) ?></strong> <?php echo $ctype ? $ctype->label : $content->item_type.' ('.__( 'not found', 'bp-moderation' ).')';
				?><strong><?php _e( 'Item ID:', 'bp-moderation' ) ?></strong> <?php echo $content->item_id.($content->item_id2?' - '.$content->item_id2:'')
				?><strong><?php _e( 'Date:', 'bp-moderation' ) ?></strong> <?php echo $this->date_abbr($content->item_date) ?>

				<div class="row-actions">
					<a class="view-content vim-v" href="<?php echo $content->item_url ?>" title="<?php _e( 'View this content', 'bp-moderation' ) ?>"><?php _e( 'View', 'bp-moderation' ) ?></a>
<?php			if ('ignored'!=$content->status): ?>
					| <a class="ignore-content vim-a" href="<?php echo wp_nonce_url( "admin.php?bpmod-action=ignore&cont_id=$content->content_id" , 'ignore' ) ?>" title="<?php _e( 'Mark this content as clean', 'bp-moderation' ) ?>"><?php _e( 'Ignore', 'bp-moderation' ) ?></a>
<?php			endif;
				if ($ctype && $ctype->callbacks['edit'] && 'deleted'!=$content->status): ?>
					| <a class="edit-content vim-e" href="<?php echo wp_nonce_url( "admin.php?bpmod-action=edit&cont_id=$content->content_id" , 'edit' ) ?>" title="<?php _e( 'Link to the edit page for this content', 'bp-moderation' ) ?>"><?php _e( 'Edit', 'bp-moderation' ) ?></a>
<?php			endif;
				if ('deleted'!=$content->status && 'moderated'!=$content->status): ?>
					| <a class="mark-moderated vim-m" href="<?php echo wp_nonce_url( "admin.php?bpmod-action=mark_moderated&cont_id=$content->content_id" , 'mark_moderated' ) ?>" title="<?php _e( 'Manually mark this content as moderated if it has already been edited/deleted without this plugin', 'bp-moderation' ) ?>" ><?php _e( 'Mark as moderated', 'bp-moderation' ) ?></a>
<?php			endif;
				if ($ctype && $ctype->callbacks['delete'] && 'deleted'!=$content->status): ?>
					| <a class="delete-content vim-d" href="<?php echo wp_nonce_url( "admin.php?bpmod-action=delete&cont_id=$content->content_id" , 'delete' ) ?>" title="<?php _e( 'Delete this content', 'bp-moderation' ) ?>"><?php _e( 'Delete', 'bp-moderation' ) ?></a>
<?php			endif; ?>
				</div>
			</td>
...
<?php
	...
	function author_details($uid, $content = false){
		if( $uid && $user = get_userdata($uid) ){ //id refers to an existing user
			$details = array(
				...
				'user_link'    => $content?$this->show_in_users_table_link($uid):bp_core_get_userlink($uid),
			...
			$details = array(
				...
				'user_link'    => $content?$this->show_in_users_table_link($uid):sprintf(__('User #%d','bp-moderation'),$uid),
				'contact_link' => ''
			);
		}else{
			$details = array(
	...
	function show_in_users_table_link( $uid ){
		...
		$title = $username ? __( 'See more details on this user in the users table', 'bp-moderation' ) : __( 'This user is not registered anymore, but you can still see some details on it in the users table', 'bp-moderation' );
		if ( !$username ) $username = sprintf(__('User #%d','bp-moderation'),$uid);
	...
	function query_contents(){
		...
		if ( isset($_GET['order']) && is_array($_GET['order']) )
			foreach ($_GET['order'] as $o) {
				if( !isset($o['by']) || !in_array($o['by'], $can_order) ||
					!isset($o['dir']) || ( 'ASC'!=$o['dir'] && 'DESC'!=$o['dir'] ) )
					continue;
				$orders[] = $o['by'].' '.$o['dir'];
			}
		if( count($orders) )
		...
		//limits
		$per_page = (isset($_GET['per_page']) && intval($_GET['per_page'])) ? intval($_GET['per_page']) : 20;
		$page_index = (isset($_GET['cpage']) && intval($_GET['cpage'])) ? intval($_GET['cpage'])-1 : 0;
	...
	function view_users(){
		...
		$sel = ' selected="selected"';
	?>
	<form id="bpmod-users-query" class="bpmod-form-query" action="admin.php" method="get">
		<input type="hidden" name="page" value="bp-moderation" />
		...
		<fieldset>
			...
			<div class="column">
				<h4><?php _e('Filters','bp-moderation') ?></h4>
				<dt>
					<input id='filter-user' <?php echo isset($_GET['active_filters']['user']) ? $chk : ''
					?> name='active_filters[user]' type='checkbox' />
					<label for='filter-user' ><?php _e('Specific users','bp-moderation') ?></label>
				</dt>
				<dd>
					<input id='user' class='line' size='40' type='text' name='filters[user]' value='<?php
						echo empty($_GET['filters']['user']) ? '': $_GET['filters']['user'] ?>' />
					<label for='user' ><?php _e('User ids (comma separeted)','bp-moderation') ?></label>
				</dd>
<?php		
			$filters = array(
				array(
...
?>
				<dt>
					<input id='filter-<?php echo $slug ?>' <?php echo checked('on',@$_GET['active_filters'][$slug])
					?> name='active_filters[<?php echo $slug ?>]' type='checkbox' />
					<label for='filter-<?php echo $slug ?>' ><?php echo $title ?></label>
				</dt>
				<dd>
					<label for='<?php echo $slug ?>' ><?php echo sprintf($desc,
...
?>
					...
					$asc = 'DESC' == @$_GET['order'][$i]['dir'] ? 'DESC' : 'ASC' ; ?>
						...
						<select name="order[<?php echo $i ?>][by]">
							<option<?php selected('none',$orby) ?> value="none"><?php _e('none','bp-moderation') ?></option>
							<option<?php selected('own_contents',$orby) ?> value="own_contents"><?php _e('total own contents reported','bp-moderation') ?></option>
							<option<?php selected('own_new',$orby) ?> value="own_new"><?php _e('pending own contents') ?></option>
							<option<?php selected('own_ignored',$orby) ?> value="own_ignored"><?php _e('ignored own contents') ?></option>
							<option<?php selected('own_moderated',$orby) ?> value="own_moderated"><?php _e('moderated own contents') ?></option>
							<option<?php selected('own_flags',$orby) ?> value="own_flags"><?php _e('total flags on own contents') ?></option>
							<option<?php selected('others_contents',$orby) ?> value="others_contents"><?php _e('total contents reported by user','bp-moderation') ?></option>
							<option<?php selected('others_new',$orby) ?> value="others_new"><?php _e('pending contents reported by user','bp-moderation') ?></option>
							<option<?php selected('others_ignored',$orby) ?> value="others_ignored"><?php _e('ignored contents reported by user','bp-moderation') ?></option>
							<option<?php selected('others_moderated',$orby) ?> value="others_moderated"><?php _e('moderated contents reported by user','bp-moderation') ?></option>
						</select>
						<select name="order[<?php echo $i ?>][dir]">
							<option<?php selected('ASC',$asc) ?> value="ASC">ASC</option>
							<option<?php selected('DESC',$asc) ?> value="DESC">DESC</option>
...
?>
				...
				<p><label for='limit' ><?php
					$input = "<input id='limit' size='4' type='text' name='per_page' value='". (empty($_GET['per_page']) ? '20': $_GET['per_page']) ."' />";
					echo sprintf(__('Display at most %s users','bp-moderation'),$input); ?></label></p>
				<input name="submit" type="submit" class="button-primary" value="<?php _e('Query Users','bp-moderation'); ?>" />
			</div>
		</fieldset>
	</form>
	<div class="clear"></div>
...
?>
	<form id="bpmod-users-form" class="bpmod-bulk-form" action="admin.php" method="post">
	<div class="tablenav">
		<div class="alignleft actions">
			<select name="bulk-action">
				<option value="-1" selected="selected"><?php _e('Bulk Actions','bp-moderation') ?></option>
				<option value="mark_spammer"><?php _e('Mark users as spammers','bp-moderation'); ?></option>
				<option value="unmark_spammer"><?php _e('Mark users as not spammers','bp-moderation'); ?></option>
			</select>
			<input type="hidden" name="bpmod-action" value="bulk_users" />
	...
	<div class="clear"></div>
	<table id="bpmod-users-table" class="widefat bpmod-table fixed" cellspacing="0">
	<thead>
		<tr>
			<th class="manage-column column-cb check-column" scope="col"><input type="checkbox"></th>
			<th class="manage-column column-author" scope="col"><?php _e('User','bp-moderation') ?></th>
			<th class="manage-column column-own-contents" scope="col"><?php _e('Own contents reported by others','bp-moderation') ?></th>
			<th class="manage-column column-other-contents" scope="col"><?php _e('Contents reported by user','bp-moderation') ?></th>
		</tr>
	</thead>
	<tfoot>
		<tr>
...
?>
		<tr class="">
			<th class="check-column" scope="row"><input type="checkbox" value="<?php echo $user->user_id ?>" name="bulk_items[]"></th>
			<td class="column-author">
				<strong><?php echo $author['avatar_img'].$author['user_link'] ?></strong>
				<br><?php echo $author['contact_link'] ?>
				<div class="row-actions">
...
<?php		elseif (bp_core_is_user_spammer( $user->user_id )): ?>
					<a class="unmark-spammer vim-u" href="<?php echo wp_nonce_url( "admin.php?bpmod-action=mark_unmark_spammer&user_id=$user->user_id&set_spam=0" , 'mark_unmark_spammer' ) ?>" title="<?php _e( 'Mark the author of this content as not spammer', 'bp-moderation' ) ?>"><?php _e( 'Mark as not spammer', 'bp-moderation' ) ?></a>
<?php		else : ?>
					<a class="mark-spammer vim-s" href="<?php echo wp_nonce_url( "admin.php?bpmod-action=mark_unmark_spammer&user_id=$user->user_id&set_spam=1" , 'mark_unmark_spammer' ) ?>" title="<?php _e( 'Mark the author of this content as spammer', 'bp-moderation' ) ?>"><?php _e( 'Mark as spammer', 'bp-moderation' ) ?></a>
<?php		endif; ?>
				</div>
			</td>
		...
		if	($user->own_contents):?>
				...
				<br/><strong><?php _e( 'New:', 'bp-moderation' ) ?></strong> <?php echo $user->own_new;
				...
				<div class="row-actions">
					<a class="vim-b" href="admin.php?page=bp-moderation&amp;view=contents&amp;filters[active_filters][item_author]=on&amp;filters[item_author]=<?php echo $user->user_id ?>" title="<?php _e( 'Show the contents from this user that have been reported in the contents view', 'bp-moderation' ) ?>"><?php _e( 'Show in contents view', 'bp-moderation' ) ?></a>
				</div>
		...
		if	($user->others_contents):?>
				...
				<br/><strong><?php _e( 'New:', 'bp-moderation' ) ?></strong> <?php echo $user->others_new;
				...
				<div class="row-actions">
					<a class="vim-g" href="admin.php?page=bp-moderation&amp;view=contents&amp;filters[active_filters][reporters]=on&amp;filters[reporters]=<?php echo $user->user_id ?>" title="<?php _e( 'Show the contents from this user that have been reported in the contents view', 'bp-moderation' ) ?>"><?php _e( 'Show in contents view', 'bp-moderation' ) ?></a>
				</div>
...
<?php
	...
	function query_users(){
		...
		//limits
		$per_page = (isset($_GET['per_page']) && intval($_GET['per_page'])) ? intval($_GET['per_page']) : 20;
		$page_index = (isset($_GET['page']) && intval($_GET['page'])) ? intval($_GET['page'])-1 : 0;
		$limits = 'LIMIT '.( $page_index * $per_page).', '.$per_page;
...
// JLL_MOD - fixed 
	...
	function add_settings_section( $slug, $title, $description = '' ){
		$this->settings_section_texts[$slug] = $description ? "<p>$description</p>" : $description;
