 ...
 */
define( 'BP_PLUGIN_DIR', WP_PLUGIN_DIR . '/communitypress/cp-core' );
define( 'BP_PLUGIN_URL', plugins_url( $path = '/communitypress/cp-core' ) );
...
if ( file_exists( WP_PLUGIN_DIR . '/communitypress/cp-core/cp-custom-pre-op.php' ) )
	require( WP_PLUGIN_DIR . '/communitypress/cp-core/cp-custom-pre-op.php' );
...
// JLL_MOD - fixed theme dir path
/* Register BuddyPress themes contained within the bp-theme folder */
if ( function_exists( 'register_theme_directory') )
	register_theme_directory( WP_PLUGIN_DIR . '/communitypress/cp-themes' );
...
// JLL_MOD - fixed Admin tab title and subnav
	// Add the administration tab under the "Site Admin" tab for site administrators
	bp_core_add_admin_menu_page( array(
		'menu_title' => __( 'CommPress', 'buddypress' ),
		//'page_title' => __( 'CommunityPress', 'buddypress' ),
	...
	add_submenu_page( 'bp-general-settings', __( 'Customize', 'buddypress'), __( 'Customize', 'buddypress' ), 'manage_options', 'communitypress-setup', 'bp_core_communitypress_setup' );
...
function bp_core_new_nav_item( $args = '' ) {
	...
	$defaults = array(
		...
		'show_for_loggedin_user' => true, // When viewing your own profile does this nav item show up?
	...
	$bp->bp_nav[$slug] = array(
		...
		'show_for_loggedin_user' => $show_for_loggedin_user, // JLL_MOD - Added
		'position' => $position
	);
...
// JLL_MOD - modded function 
	...
	if ( empty( $wp_rewrite->permalink_structure ) ) { ?>
		<div id="message" class="updated fade">
			<p><?php printf( __( '<strong>CommunityPress is almost ready</strong>. You must <a href="%s">update your permalink structure</a> to something other than the default for it to work.', 'buddypress' ), admin_url( 'options-permalink.php' ) ) ?></p>
		</div><?php
	} elseif ( !(int)get_site_option( 'users_can_register' ) ) { ?>
		<div id="message" class="updated fade">
			<p><?php printf( __( '<strong>CommunityPress is almost ready</strong>. <a href="%s">Allow user registration... </a> check the "Anyone can register" box', 'buddypress' ), admin_url( 'options-general.php' ) ) ?></p>
		</div><?php
	} else {
		...
		if ( !defined( 'BP_SILENCE_THEME_NOTICE' ) && !in_array( 'buddypress', (array)$ct->tags ) ) { ?>
			<div id="message" class="updated fade">
				<p style="line-height: 150%"><?php printf( __( "<strong>CommunityPress is ready</strong>. You'll need to <a href='%s'>activate the CommunityPress Default theme</a> or CommunityPress won't work.", 'buddypress' ), admin_url( 'themes.php' ) ) ?></p>
			</div><?php
		}
	}
}
