	<?php get_header('buddypress'); ?>
		<div id="content" class="clearfix">
			<div id="content-area">			
				<div class="entry_buddypress clearfix">

		<?php do_action( 'bp_before_member_home_content' ) ?>

		<div id="item-header">
			<?php locate_template( array( 'members/single/member-header.php' ), true ) ?>
		</div><!-- #item-header -->

		<div id="item-nav">
			<div class="item-list-tabs no-ajax" id="object-nav">
				<ul>
					<?php if ( bp_is_my_profile() ) : ?>
                        <?php bp_get_loggedin_user_nav() ?>
                    <?php else: ?>
                        <?php bp_get_displayed_user_nav() ?>
                    <?php endif; ?>

					<?php do_action( 'bp_members_directory_member_types' ) ?>
				</ul>
			</div>
		</div><!-- #item-nav -->

		<div id="item-body">
			<?php do_action( 'bp_before_member_body' ) ?>
            
            
           
            
            
			<?php // >>>>>----->>>>> JLL MOD <<<<<------<<<<< ?>
            
			<?php if ( 'wall' == bp_current_component() ) : ?>
				<?php if ( bp_privacy_filter("wall") ) : ?>
					<?php locate_template( array( 'members/single/not-friend.php' ), true ) ?>
				<?php else : ?>
					<?php locate_template( array( 'members/single/wall.php' ), true ) ?>
                <?php endif; ?>

			<?php elseif ( bp_is_user_profile() ) : ?>
				<?php if ( bp_privacy_filter("info") ) : ?>
                    <?php locate_template( array( 'members/single/not-friend.php' ), true ) ?>
                <?php else : ?>
					<?php locate_template( array( 'members/single/profile.php' ), true ) ?>
                <?php endif; ?>
                
			<?php //elseif ( bp_is_user_blogs() ) : ?>
				<?php //locate_template( array( 'members/single/blogs.php' ), true ) ?>

			<?php elseif ( bp_is_user_friends() ) : ?>
				<?php if ( bp_privacy_filter("friends") ) : ?>
                    <?php locate_template( array( 'members/single/not-friend.php' ), true ) ?>
                <?php else : ?>
                	<?php locate_template( array( 'members/single/friends.php' ), true ) ?>
                <?php endif; ?>

			<?php elseif ( bp_is_user_groups() ) : ?>
				<?php if ( bp_privacy_filter("groups") ) : ?>
                    <?php locate_template( array( 'members/single/not-friend.php' ), true ) ?>
                <?php else : ?>
					<?php locate_template( array( 'members/single/groups.php' ), true ) ?>
                <?php endif; ?>

			<?php elseif ( bp_is_user_messages() ) : ?>
				<?php locate_template( array( 'members/single/messages.php' ), true ) ?>

			<?php // >>>>>----->>>>> JLL MOD <<<<<------<<<<< ?>
			<?php elseif ( bp_is_user_activity() && 'all' == bp_current_action() || '' == bp_current_action() ) : ?>
				<?php locate_template( array( 'members/single/activity-all.php' ), true ) ?>

			<?php elseif ( bp_is_user_activity() ) : ?>
				<?php locate_template( array( 'members/single/activity.php' ), true ) ?>

				<?php else : ?>
					<?php
						/* If nothing sticks, just load a member front template if one exists. */
						locate_template( array( 'members/single/wall.php' ), true );
					?>



			<?php endif; ?>

			<?php do_action( 'bp_after_member_body' ) ?>

		</div><!-- #item-body -->

		<?php do_action( 'bp_after_member_home_content' ) ?>

				</div> <!-- end .entry -->		
			</div> <!-- end #content-area -->	
	<?php get_sidebar('BP-Profile'); ?>
		</div> <!-- end #content --> 
	<?php get_footer(); ?>
