...
function bp_album_screen_single() {
	...
	function bp_album_screen_edit_content() {
		...
		?>		
		<h4><?php _e( 'Edit Photo', 'bp-album' ) ?></h4>
					...
					$(document).ready(function(){
                        $("#picture-edit-thumb").tag({
							...
							save : function( width,height,top_pos,left,label,the_tag,fid ){
											...
											var fid = $("#fid").html(); 
											var oid = "<?php global $bp; echo $bp->displayed_user->id; ?>";
											...
											xmlhttp.open("GET","<?php global $bp; echo $bp->root_domain; ?>/wp-content/plugins/communitypress/cp-modules/cp-photo-tag/phototag-gethint.php?wid="+width+"&hei="+height+"&top="+top_pos+"&left="+left+"&lab="+label+"&pid="+pid+"&fid="+fid+"&oid="+oid,true);
											xmlhttp.send();
										},
							remove: function(id){
											...
											xmlhttp.open("GET","<?php global $bp; echo $bp->root_domain; ?>/wp-content/plugins/communitypress/cp-modules/cp-photo-tag/phototag-gethint.php?d="+id+"&pid=<?php bp_album_picture_id(); ?>",true);
											xmlhttp.send();
										},
                        });
