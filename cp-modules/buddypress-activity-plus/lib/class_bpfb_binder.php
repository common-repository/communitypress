...
class BpfbBinder {
	...
	function js_load_scripts () {
		...
		wp_localize_script('bpfb_interface_script', 'l10nBpfb', array(
			...
			'paste_video_url' => __('http:// Paste video URL here', 'bpfb'),
			'paste_link_url' => __('http:// Paste link here', 'bpfb'),
		));
	}
	...
	function ajax_preview_video () {
		$url = $_POST['data'];
		$warning = __('Oops, there was an error, did you include http:// ?', 'bpfb');
	...
	function _add_js_css_hooks () {
		...
		if (
			...
			// JLL_MOD - Load the script on Profile Wall page too
			||
			('wall' == $bp->current_component)
