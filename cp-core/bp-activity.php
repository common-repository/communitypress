...
// JLL_MOD - Custom Activity navigation
function bp_activity_setup_nav() {
	global $bp;

	/* Add 'Activity' to the main navigation */
	bp_core_new_nav_item( array( 'name' => __( 'News Feed', 'buddypress' ), 'slug' => $bp->activity->slug, 'position' => 10, 'show_for_displayed_user' => false, 'screen_function' => 'bp_activity_screen_my_activity', 'default_subnav_slug' => 'all', 'item_css_id' => $bp->activity->id ) );
	...
	bp_core_new_subnav_item( array( 'name' => __( 'All', 'buddypress' ), 'slug' => 'all', 'parent_url' => $activity_link, 'parent_slug' => $bp->activity->slug, 'screen_function' => 'bp_activity_screen_all_activity', 'position' => 10 ) );
	...
	bp_core_new_subnav_item( array( 'name' => sprintf( __( 'Posts to your Wall', 'buddypress' ), $user_login ), 'slug' => 'mentions', 'parent_url' => $activity_link, 'parent_slug' => $bp->activity->slug, 'screen_function' => 'bp_activity_screen_mentions', 'position' => 50, 'item_css_id' => 'activity-mentions' ) );
...
function bp_activity_format_notifications( $action, $item_id, $secondary_item_id, $total_items ) {
	...
	switch ( $action ) {
		case 'new_at_mention':
			...
			//$act = new BP_Activity_Activity($item_id);
			$wall_poster_id = $secondary_item_id;
			$wall_poster_name = bp_core_get_user_displayname( $wall_poster_id );
			//$wall_post_link = bp_core_get_user_domain( $wall_poster_id ) . $bp->wall->slug . '/view/';
			$wall_post_link = $bp->displayed_user->domain . 'wall/';
			if ( (int)$total_items > 1 ) {
				return apply_filters( 'new_wall_posts_notification', '<a href="' . $wall_post_link . '" title="New posts to your wall">' . sprintf( '%d new posts on your wall', (int)$total_items ) . '</a>', $wall_post_link, $total_items );
			} else {
				return apply_filters( 'new_wall_post_notification', '<a href="' . $wall_post_link . '" title="Someone wrote on your wall">' . sprintf( '%s wrote on your wall', $wall_poster_name ) . '</a>', $wall_post_link );
			}
		break;
		
	}

	do_action( 'bp_activity_format_notifications', $action, $item_id, $secondary_item_id, $total_items );
...
// JLL_MOD - Add screen func for custom navigation
