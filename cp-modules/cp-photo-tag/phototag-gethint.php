...
if (!empty($_GET["d"])){
	...
	if (!empty($tagfields["tagged_id"])){
		...
		foreach ( $photo as $pic ) {
				...
				bp_activity_add( array( 'user_id' => $tagfields["tagged_id"], 'action' => $action, 'content' => $content, 'primary_link' => $primary_link, 'component' => 'album', 'type' => $type, 'item_id' => $tagid ) );
		}
		// Nitification second
