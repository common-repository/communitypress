...
?>
	<div class="wrap">
		...
		if ( isset( $_REQUEST['reinstall'] ) || !bp_forums_is_installed_correctly() ) {
			...
			<p><?php printf( __( 'bbPress forum integration in CommunityPress has been set up correctly. If you are having problems you can <a href="%s" title="Reinstall bbPress">re-install</a>', 'buddypress' ), $reinstall ); ?>
	...
	switch( $_REQUEST['step'] ) {
		case 'existing':
			if ( 1 == (int)$_REQUEST['doinstall'] ) {
				if ( !bp_forums_configure_existing_install() ) {
					...
					<p><?php _e( 'CommunityPress will now use its internal copy of bbPress to run the forums on your site. If you wish, you can remove your old bbPress installation files, as long as you keep the bb-config.php file in the same location.', 'buddypress' ) ?></p><?php
				}
			} else {
				?>
					<form action="" method="post">
						<h3><?php _e( 'Existing bbPress Installation', 'buddypress' ) ?></h3>
						<p><?php _e( "CommunityPress can make use of your existing bbPress install. Just provide the location of your <code>bb-config.php</code> file, and CommunityPress will do the rest.", 'buddypress' ) ?></p>
		...
		case 'new':
			...
			?>
				...
				<p><?php _e( "You've decided to set up a new installation of bbPress for forum management in CommunityPress. This is very simple and is usually just a one click
		...
		default:
			...
			<?php } else { ?>
				...
				<p><?php _e( 'Forums in CommunityPress make use of a bbPress installation to function. You can choose to either let CommunityPress set up a new bbPress install, or use an already existing bbPress install. Please choose one of the options below.', 'buddypress' ) ?></p>
