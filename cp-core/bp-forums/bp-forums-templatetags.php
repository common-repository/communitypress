...
function bp_directory_forums_search_form() {
	...
	$search_value = __( ' Search forums...', 'buddypress' );
	if ( !empty( $_REQUEST['fs'] ) )
	 	$search_value = $_REQUEST['fs'];

?>
	<form action="" method="get" id="search-forums-form">
		<label><input type="text" name="s" id="forums_search" value="<?php echo esc_attr($search_value) ?>"  onfocus="if (this.value == '<?php _e( ' Search forums...', 'buddypress' ) ?>') {this.value = '';}" onblur="if (this.value == '') {this.value = '<?php _e( ' Search forums...', 'buddypress' ) ?>';}" /></label>
