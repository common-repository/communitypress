...
function friends_setup_nav() {
	...
	// JLL_MOD - Add 'Invite Friends' to Friend Nav Tab
	global $wpdb;
	$disabled_components = get_site_option( 'bp-deactivated-components' );
	if ( !isset( $disabled_components['fbconnect'] ) ){
	bp_core_new_subnav_item( array( 'name' => __( 'Invite Friends', 'buddypress' ), 'slug' => 'invitefriends', 'parent_url' => $friends_link, 'parent_slug' => $bp->friends->slug, 'screen_function' => 'invite_screen', 'position' => 30, ) );
	}
...
// JLL_MOD - Add screen func for custom nav
function invite_screen ()
{
	global $bp;
$location= $bp->root_domain.'/invite-friends/';
bp_core_load_template( 'fbinvites', true);
}
