...
function xprofile_install() {
	...
	// JLL_MOD - fixed
	bp_core_activate_site_options( array( 'bp-xprofile-base-group-name' => 'Basic Info', 'bp-xprofile-fullname-field-name' => 'Display Name' ) );
...
// JLL_MOD - fixed Profile Navigation and repositioned profile
	/* Add 'Profile' to the main navigation */
	bp_core_new_nav_item( array( 'name' => __( 'Info', 'buddypress' ), 'slug' => $bp->profile->slug, 'position' => 35, 'show_for_dispalyed_user' => false, 'show_for_loggedin_user' => false, 'screen_function' => 'xprofile_screen_display_profile', 'default_subnav_slug' => 'info', 'item_css_id' => $bp->profile->id ) );

	$profile_link = $bp->loggedin_user->domain . $bp->profile->slug . '/';

	/* Add the subnav items to the profile */
	bp_core_new_subnav_item( array( 'name' => __( 'View Info', 'buddypress' ), 'slug' => 'info', 'parent_url' => $profile_link, 'parent_slug' => $bp->profile->slug, 'screen_function' => 'xprofile_screen_display_profile', 'position' => 10 ) );
	bp_core_new_subnav_item( array( 'name' => __( 'Edit Info', 'buddypress' ), 'slug' => 'edit', 'parent_url' => $profile_link, 'parent_slug' => $bp->profile->slug, 'screen_function' => 'xprofile_screen_edit_profile', 'position' => 20 ) );
	bp_core_new_subnav_item( array( 'name' => __( 'Change Profile Picture', 'buddypress' ), 'slug' => 'change-avatar', 'parent_url' => $profile_link, 'parent_slug' => $bp->profile->slug, 'screen_function' => 'xprofile_screen_change_avatar', 'position' => 30 ) );
...
// JLL_MOD -  Customized the below function
/**
...
// JLL_MOD -  Moved function to bp-core-adminbar.php and customized
...
function xprofile_screen_display_wall() {
	global $bp;

	do_action( 'xprofile_screen_display_wall', $_GET['new'] );
	bp_core_load_template( apply_filters( 'xprofile_template_display_wall', 'members/single/home' ) );
}
...
function xprofile_remove_screen_notifications() {
	...
	bp_core_delete_notifications_for_user_by_type( $bp->loggedin_user->id, 'activity', 'new_at_mention' );
}
add_action( 'bp_activity_screen_mentions', 'xprofile_remove_screen_notifications' );
