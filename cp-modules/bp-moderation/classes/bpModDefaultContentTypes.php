...
// JLL_MOD - commented out some admin options
...
//			add_filter( 'bp_activity_get_comments', array(__CLASS__,'activity_comments_insert_links') );
//		}
 ...
 */
	...
	//this could be much easier with an appropriate hook in the right place
	function activity_comments_insert_links( $comments_html ){
		//this regex extract activity id and author id from the html and the callback append the link to the meta
		$pattern = '#(<li id="acomment-)(\d*)(.*?)(user-)(\d*)(-avatar)(.*?)(<div class="acomment-meta">)(.*?)(</div>)#';

		return preg_replace_callback($pattern, array(__CLASS__,'activity_comment_replace_callback'), $comments_html);
	}
		function activity_comment_replace_callback( $m ){
			...
			if ($link)
				return $m[1].$m[2].$m[3].$m[4].$m[5].$m[6].$m[7].$m[8].$m[9].' &middot; '.$link.$m[10];
			else
				return $m[0];
		}
 ...
 */
	...
	function blog_comment_author_details($details,$cont){
		...
		$details = array(
				...
				'contact_link' => $email?"<a class='vim-c' href='mailto:$email' title='".__( 'Send an email to the author of this content', 'bp-moderation' )."' >".__( 'Send email', 'bp-moderation' )."</a>":''
			);
