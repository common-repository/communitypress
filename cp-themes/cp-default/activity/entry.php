...
<li class="<?php bp_activity_css_class() ?>" id="activity-<?php bp_activity_id() ?>">
	...
	<div class="activity-content">
		...
		<div class="activity-header">
			...
			global $bp;
            $actcontent = bp_get_activity_content_body();
			$pre_action = bp_get_activity_action();
			$displayeduserid = $bp->displayed_user->id;
			$loggedinuserid = $bp->loggedin_user->id;

			preg_match('%/members/(.+?)/"%', $pre_action, $mentioner);
			preg_match_all( '/[@]+([A-Za-z0-9-_\.]+)/', $actcontent, $mentionmatch );
			$poster = $mentioner[1];
			$postee = $mentionmatch[1][0];
			$posterid = bp_core_get_userid( $poster );
			$posteeid = bp_core_get_userid( $postee );
			$posterlink = bp_core_get_userlink($posterid);
			$posteelink = bp_core_get_userlink($posteeid);

				if ( !empty( $postee ) ) {
					if ( $loggedinuserid == $posteeid ) {
					echo $posterlink.' posted on your wall: ';
					echo bp_insert_activity_meta('');
					} else if ( $loggedinuserid == $posterid ) {
					echo 'You posted on '.$posteelink."'s wall: ";
					echo bp_insert_activity_meta('');
					} else {
					echo $posterlink.' posted on '.$posteelink."'s wall: ";
					echo bp_insert_activity_meta('');
					}
				} else { 
