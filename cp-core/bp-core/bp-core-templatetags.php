...
function bp_directory_members_search_form() {
	...
	$search_value = __( ' Search members...', 'buddypress' );
	if ( !empty( $_GET['s'] ) )
	 	$search_value = $_GET['s'];

	?>
	<form action="" method="get" id="search-members-form">
		<label><input type="text" name="s" id="members_search" value="<?php echo esc_attr( $search_value ) ?>"  onfocus="if (this.value == '<?php _e( ' Search members...', 'buddypress' ) ?>') {this.value = '';}" onblur="if (this.value == '') {this.value = '<?php _e( ' Search members...', 'buddypress' ) ?>';}" /></label>
...
function bp_get_loggedin_user_nav() {
	...
	foreach( (array) $bp->bp_nav as $nav_item ) {
		...
		// JLL_MOD - added if conditional to hide nav items for loggedin users
		if ( !$nav_item['show_for_loggedin_user'] )
			continue;
...
function bp_get_displayed_user_nav() {
	...
	foreach ( (array)$bp->bp_nav as $user_nav_item ) {
		...
		if ( $bp->current_component == $user_nav_item['slug'] )
			...
			$selected = '';
// JLL_MOD - highlight wall tab for custom redirection mods
		if ( $bp->current_component == 'activity' && 'wall' == $user_nav_item['slug'])
			$selected = ' class="current selected"';
